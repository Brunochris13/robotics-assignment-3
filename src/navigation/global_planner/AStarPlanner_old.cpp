...
**/
  ...
  {
    ROS_INFO("Start of runAStarOnGrid");
    vector<int> bestPath;

    ROS_INFO("mapSize= %d", mapSize);
    std::map<int, float> g_score_map;
    // for (int i = 0; i < mapSize; i++)
    // {
    //   if (isFree(i))
    //     g_score_map[i] = infinity;
    // }
    ...
    ROS_INFO("Before findPath");
    // bestPath = findPath(startGridSquare, goalGridSquare, g_score);
    bestPath = findPath(startGridSquare, goalGridSquare, g_score_map);
...
**/
  ...
  vector<int> AStarPlanner::findPath(int startGridSquare, int goalGridSquare, std::map<int, float> &g_score)
  {
    ROS_INFO("Here 0");
    ...
    visited.push_back(startGridSquare);
    ...
    while (!openSquaresList.empty() && (g_score.count(goalGridSquare) == 0))
    {
    ...
    if (g_score.count(goalGridSquare) > 0) // if goal gridSquare has been reached
    {
      ROS_INFO("Before constructing a Path");
      bestPath = constructPath(startGridSquare, goalGridSquare, g_score);
      ROS_INFO("Finished constructing a Path");
      return bestPath;
    }
    else
    {
      ROS_WARN("Failure to find a path !");
      return emptyPath;
    }
  }
...
**/
  ...
  void AStarPlanner::addNeighborGridSquareToOpenList(multiset<GridSquare> &openSquaresList, int neighborGridSquare, int goalGridSquare, std::map<int, float> g_score)
  {
    GridSquare gridSq;
