...
class Communication:
    ...
    Attributes:
        name (str): The full name of this part (robot + communication)
        FNULL (TextIOWrapper): The IO wrapper pointing to `/dev/null`
    """
    def __init__(self, basename=""):
        """Initializes the communication part.

        Args:
            basename (str, optional): The name of the robot this part
    ...
    def record(self, filepath, dur=3):
        ...
        Args:
            ...
            dur (int, optional): The duration of the recording in
    ...
    def recognize_voice(self, path=None):
        ...
        Args:
            path (str, optional): The path to audio file to recognize.
                Defaults to `None`.

        Returns:
            tuple(Action, str): A tuple consisting of action and voice
                data
        """
    ...
    def listen(self, max_prompts=3, source=None, dur=5):
        ...
        Args:
            max_prompts (int, optional): [description]. Defaults to 3.
            source ([type], optional): [description]. Defaults to None.
            dur (int, optional): [description]. Defaults to 5.
    ...
    def ask(self, message, valid_answers=[], binary=False, max_prompts=(3, 3)):
        ...
        while action != Action.BASE.ACCEPT and num_prompts < max_prompts[1]:
            ...
            # Ask the question and wait until some answer is acquired
            voice_data = self.say_listen(message, binary, max_prompts[0])
            rospy.loginfo(self.name + "Acquired answer: " + str(voice_data))

            if voice_data not in valid_answers and \
               num_prompts < max_prompts[1]:
